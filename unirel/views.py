...
def admin(request, year):
    ...
    for participant in participants:
        if participant.addon_banquet:
            count_banquet += 1
        if participant.addon_sleep:
            count_sleep += 1
        if participant.addon_lunch:
            count_lunch += 1
...
def admin_dietary_restrictions(request, year):
    ...
    participants_all = Participant.objects.prefetch_related(
    ...
    if request.POST and form.is_valid():
        if form.cleaned_data["addon_sleep"]:
            participants_all = participants_all.exclude(addon_sleep=False)
        if form.cleaned_data["addon_lunch"]:
            participants_all = participants_all.exclude(addon_lunch=False)
        if form.cleaned_data["addon_banquet"]:
            participants_all = participants_all.exclude(addon_banquet=False)
    ...
    for participant in participants_all:
        for dietary_restriction in participant.dietary_restrictions.all():
            if dietary_restriction in dietary_restrictions_all:
                dietary_restrictions_all[dietary_restriction].append(participant)
            else:
                dietary_restrictions_all[dietary_restriction] = [participant]

    participants = []

    for participant in participants_all:
        l = [
            (participant in dietary_restrictions_all[x])
            for x in dietary_restrictions_all
        ]

        participants.append({"participant": participant, "dietary_restrictions": l})

    return render(
        request,
        "unirel/admin_dietary_restrictions.html",
        {
            "fair": fair,
            "form": form,
            "participants": participants,
            "dietary_restrictions": [
                {"name": x, "count": len(dietary_restrictions_all[x])}
                for x in dietary_restrictions_all
            ],
        },
    )
...
def admin_participant(request, year, participant_pk=None):
    fair = get_object_or_404(Fair, year=year)
    participant = (
        get_object_or_404(Participant, pk=participant_pk, fair=fair)
    ...
    return render(
        request,
        "unirel/admin_participant.html",
        {
            "fair": fair,
            "participant": participant,
            "url": request.build_absolute_uri(
                reverse("unirel_register", args=[participant.token])
            ),
        },
    )
...
def admin_participant_form(request, year, participant_pk=None):
    ...
    return render(
        request, "unirel/admin_participant_form.html", {"fair": fair, "form": form}
    )
...
def register(request, token):
    ...
    return render(
        request, "unirel/register.html", {"participant": participant, "form": form}
    )
