...
class ExhibitorCreateBypassedForm(forms.Form):
	company = forms.ModelChoiceField(queryset = Company.objects.all(), widget = forms.Select(), required = True, label = 'Select a company that you want to add as an exhibitor.')
