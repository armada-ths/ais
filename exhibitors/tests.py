...
class ExhibitorTestCase(TestCase):
    def setUp(self):
        ...
        fairOld = Fair.objects.create(
            name="Armada Old", year=2016, current=False, pk=99133799
        )
        ...
        contact = Contact.objects.create(
            user=user, belongs_to=company, email=user.email
        )

        exhibitor = Exhibitor.objects.create(
            fair=fair, company=company, contact=contact, pk=1
        )

        product1 = Product.objects.create(
            fair=fair, name="product1", coa_number=5, price=100
        )
        product2 = Product.objects.create(
            fair=fair, name="product2", coa_number=5, price=200
        )
    ...
    def testEmailFunction(self):
        ...
        adminUser = User.objects.create(
            username="admin", email="admin@admin.com", is_superuser=True
        )
    ...
    def testExhibitorView(self):
        ...
        response = client.post(
            "/fairs/2017/exhibitors/view",
            {"order": True, "fair": True, "booth_number": True},
        )
