...
def form_complete(request, company, company_contact, fair, exhibitor):
	...
	formset_company_address = CompanyAddressFormSet(request.POST if request.POST and request.POST.get('save_company_details') else None, instance = company, prefix = 'form_address')
	...
	if request.POST:
		if request.POST.get('save_company_details') and form_company_details.is_valid(company) and formset_company_address.is_valid():
		...
		elif request.POST.get('save_final_submission') and form_final_submission.is_valid() and signature is None and company_contact is not None:
