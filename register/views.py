...
def get_contract(company, fair, registration_type):
    ...
    if not contract:
        try:
            ...
            try:
                contract = SignupContract.objects.get(fair=fair,
                                                      type=registration_type,
                                                      default=True)
            except SignupContract.DoesNotExist:
                contract = None
...
def send_CR_confirmation_email(signature, deadline):
    html_message = '''
		<html>
        	<body>
        		...
        		<div>
        		      Thank you for submitting the complete registration for THS Armada 2021. The complete registration contract was signed by %s on the %s for %s.
                      <br/><br/>
    ...
    plain_text_message = '''Thank you for submitting the complete registration for THS Armada 2021. The complete registration contract was signed by %s on the %s for %s.
...
''' % (
    ...
    email = EmailMultiAlternatives(
        'Complete registration for THS Armada 2021',
        plain_text_message,
        'noreply@armada.nu',
...
def choose_company(request):
    ...
    company_contacts = CompanyContact.objects.filter(
        user=request.user).exclude(company=None)

    if len(company_contacts) == 1:
        return redirect('anmalan:registration',
                        company_contacts.first().company.pk)
...
def form(request, company_pk):
    ...
    if timezone.now() < fair.registration_start_date:
        return render(request, 'register/inside/error_before_initial.html', {
            'fair': fair,
            'company': company,
    ...
    if timezone.now() >= fair.registration_start_date and timezone.now(
    ) < fair.complete_registration_start_date:
...
def form_initial(request, company, company_contact, fair):
    ...
    admin_user = User.objects.filter(first_name="admin",
                                     last_name="admin",
                                     email="support@armada.nu").first()
    ...
    form_initial_interests = InitialInterestsRegistrationForm(
        request.POST if request.POST
        and request.POST.get('save_initial_registration') else None)
    ...
    form_company_details = NewCompanyForm(
        request.POST
        if request.POST and request.POST.get('save_company_details') else None,
        instance=company)
    form_company_contact = InitialCompanyContactForm(
        request.POST
        if request.POST and request.POST.get('save_contact_details') else None,
        instance=company_contact)

    is_editable = timezone.now(
    ) >= fair.registration_start_date and timezone.now(
    ...
    if request.POST:
        if request.POST.get(
                'save_company_details') and form_company_details.is_valid(
                ) and is_authorized:
            ...
            if not signature:
                ...
                'save_contact_details') and form_company_contact.is_valid(
                ) and is_authorized:
            ...
            form_company_contact = InitialCompanyContactForm(
                ...
                'save_initial_registration'
        ) and is_editable and is_authorized and signature is None:
...
def form_complete(request, company, company_contact, fair, exhibitor):
    ...
    form_logistics_details = CompleteLogisticsDetailsFormWithCheckbox(
        request.POST if request.POST
        and request.POST.get('save_logistics_details') else None,
        instance=exhibitor)
    ...
    form_final_submission = CompleteFinalSubmissionForm(
        request.POST if request.POST
        and request.POST.get('save_final_submission') else None)
    ...
    is_editable = timezone.now(
    ) >= fair.complete_registration_start_date and timezone.now() <= deadline
    ...
    can_edit_orders = Order.objects.filter(
        purchasing_company=company,
        product__revenue__fair=fair).exclude(export_batch=None).count() == 0
    ...
    for registration_section_raw in RegistrationSection.objects.exclude(
        ...
        for product_raw in Product.objects.select_related('category').filter(
                revenue__fair=fair,
                registration_section=registration_section_raw):
            quantity_initial = 0
            ...
            if product_raw.max_quantity == 1:
                form_product = CompleteProductBooleanForm(
                    ...
                    initial={
                        'checkbox': True if quantity_initial == 1 else False
                    })
                ...
                form_product.fields['quantity'].choices = [
                    (i, i) for i in range(0, (
                        product_raw.max_quantity +
                        1) if product_raw.max_quantity is not None else 21)
                ]
            ...
            if request.POST and request.POST.get(
                    'save_product_' + str(product_raw.id)
            ) and form_product.is_valid() and is_editable and can_edit_orders:
                quantity = (1 if form_product.cleaned_data['checkbox'] else
                            0) if product_raw.max_quantity == 1 else int(
                                form_product.cleaned_data['quantity'])
                ...
                if quantity == 0:  # if a product has been unchecked or set to zero
                    ...
                    if product_raw.category:
                        if not product_raw.category.allow_multiple_purchases:
                            ...
                            if invalid_order:
                                ...
                                )  # will work also if invalid_order contains multiple orders
                    ...
                    if len(order_all) != 1:
                        order = Order(purchasing_company=company,
                                      product=product_raw,
                                      quantity=quantity)
            ...
            product = {
                ...
                'category':
                product_raw.category.name if product_raw.category else None,
    ...
    for order in Order.objects.filter(product__revenue__fair=fair,
                                      purchasing_company=company):
        ...
        orders.append({
            ...
            'name':
            order.product.name if order.name is None else order.name,
            'description':
            order.product.description
            if order.product.registration_section is None else None,
            'quantity':
            order.quantity,
            'product_total':
            product_total
    ...
    if request.POST:
        if request.POST.get(
            ...
            form_company_details = CompleteCompanyDetailsForm(instance=company)
                ...
                'save_logistics_details') and form_logistics_details.is_valid(
                ) and is_editable:
            ...
            form_logistics_details = CompleteLogisticsDetailsFormWithCheckbox(
                ...
                'save_catalogue_details') and form_catalogue_details.is_valid(
                ) and is_editable:
            ...
            form_catalogue_details = CompleteCatalogueDetailsForm(
                ...
                'save_final_submission') and form_final_submission.is_valid(
...
def get_ticket_deadline(fair):
    if fair.companies_ticket_deadline:
        ...
        ticket_deadline = datetime.datetime.strptime(day_string + ' 08:00',
                                                     '%Y-%m-%d %H:%M')
...
def events(request, company_pk):
    ...
    return render(
        request, 'register/inside/events.html', {
            'fair': fair,
            'company': company,
            'company_contact': company_contact,
            'exhibitor': exhibitor
