...
urlpatterns = [
    ...
    url(
        r"^login/$",
        LoginView.as_view(
            template_name="register/login.html", authentication_form=LoginForm
        ),
        name="login",
    ),
    ...
    url(
        ...
        PasswordResetView.as_view(
            ...
            email_template_name="register/outside/reset_password_email.html",
        ),
        name="password_reset",
    ),
    url(
        ...
        PasswordResetDoneView.as_view(
            template_name="register/outside/reset_password_done.html"
        ),
    ...
    url(
        ...
        PasswordResetCompleteView.as_view(
            template_name="register/outside/reset_password_complete.html"
        ),
        name="password_reset_complete",
    ),
    url(r"^(?P<company_pk>[0-9]+)/transport$", views.transport, name="transport"),
    url(
        r"^(?P<company_pk>[0-9]+)/lunchtickets$",
        views.lunchtickets,
        name="lunchtickets",
    ),
    url(
        r"^(?P<company_pk>[0-9]+)/lunchtickets/new$",
        views.lunchtickets_form,
        name="lunchtickets_new",
    ),
    url(
        r"^(?P<company_pk>[0-9]+)/lunchtickets/(?P<lunch_ticket_pk>[0-9]+)$",
        views.lunchtickets_form,
        name="lunchtickets_edit",
    ),
    url(r"^(?P<company_pk>[0-9]+)/banquet$", views.banquet, name="banquet"),
    url(
        r"^(?P<company_pk>[0-9]+)/banquet/new$", views.banquet_form, name="banquet_new"
    ),
    url(
        r"^(?P<company_pk>[0-9]+)/banquet/(?P<banquet_participant_pk>[0-9]+)$",
        views.banquet_form,
        name="banquet_edit",
    ),
]
