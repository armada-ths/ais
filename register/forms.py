...
class CompleteCompanyDetailsForm(ModelForm):
    class Meta:
        model = Company
        fields = [
            'name', 'identity_number', 'invoice_name', 'general_email_address',
            'invoice_address_line_1', 'invoice_address_line_2',
            'invoice_address_line_3', 'invoice_zip_code', 'invoice_city',
            'invoice_country', 'invoice_reference', 'invoice_email_address'
        ]

        labels = {
            'identity_number': 'Organization\'s corporate identity number',
            'general_email_address':
            'If available, please provide a non-personal e-mail address for future contact between Armada and your organization',
            'invoice_address_line_1':
            'Invoice address line 1 (required to sign contract)',
        ...
        help_texts = {
            'name':
            'Your company\'s every-day name; the name students will know you by.',
            'general_email_address':
            'This is to simplify the process of reaching out to your organization for the upcoming years\' fairs.',
            'invoice_name':
            'This should be your company\'s complete legal name.',
            'invoice_reference':
            'Name of your reference, your Purchase Order Number or equivalent.',
            'invoice_email_address':
            'If you enter an e-mail address here, the invoice will be sent only through e-mail. Leave the field blank to receive the invoice by regular mail.'
        }

    def clean(self):
        super(CompleteCompanyDetailsForm, self).clean()

        if 'invoice_zip_code' in self.cleaned_data and self.cleaned_data[
                'invoice_zip_code'] is not None and re.match(
                    r'[0-9]{5}$', self.cleaned_data['invoice_zip_code']):
            self.cleaned_data[
                'invoice_zip_code'] = self.cleaned_data['invoice_zip_code'][
                    0:3] + ' ' + self.cleaned_data['invoice_zip_code'][3:5]

        if 'invoice_email_address' in self.cleaned_data and self.cleaned_data[
                'invoice_email_address'] is not None:
            self.cleaned_data['invoice_email_address'] = self.cleaned_data[
                'invoice_email_address'].lower()

        if 'general_email_address' in self.cleaned_data and self.cleaned_data[
                'general_email_address'] is not None:
            self.cleaned_data['general_email_address'] = self.cleaned_data[
                'general_email_address'].lower()
    ...
    def is_valid(self):
        ...
        if invoice_zip_code is not None and invoice_country is not None and invoice_country == 'SWEDEN' and not re.match(
                r'[0-9]{3} [0-9]{2}$', invoice_zip_code):
            self.add_error('invoice_zip_code', 'Invalid Swedish zip code.')
            valid = False

        if invoice_zip_code is not None and invoice_country is not None and invoice_country == 'NORWAY' and not re.match(
                r'[0-9]{4}$', invoice_zip_code):
...
class CompleteLogisticsDetailsForm(ModelForm):
    class Meta:
        model = Exhibitor
        fields = [
            'booth_height', 'electricity_total_power',
            'electricity_socket_count', 'electricity_equipment',
            'placement_wish', 'placement_comment'
        ]
        ...
        widgets = {
            ...
            forms.Textarea(
                attrs={
                    ...
                    5,
                    'placeholder':
                    'We will consider your wish of placement, but we cannot give any guarantees.'
                })
        }

        labels = {
            'booth_height':
            'Height of the booth (cm) (required to sign contract)',
            'electricity_total_power':
            'Estimated power consumption (W) (required to sign contract)',
            'electricity_socket_count':
            'Number of sockets (required to sign contract)',
        }
    ...
    def is_valid(self):
        ...
        if booth_height is not None and (booth_height < 10
                                         or booth_height > 1000):
            self.add_error('booth_height',
                           'The boot height must be between 10 and 1000 cm.')
            valid = False
...
class CompleteLogisticsDetailsFormWithCheckbox(CompleteLogisticsDetailsForm):
    class Meta(CompleteLogisticsDetailsForm.Meta):
        fields = ['confirmation_box'
                  ] + CompleteLogisticsDetailsForm.Meta.fields

    confirmation_box = forms.BooleanField(
        required=False,
        label=
        '<strong>I have read the information regarding booth equipment and transportation above and I am aware of the services offered. (required to sign contract)</strong>'
    )

    def is_valid(self):
        valid = super(CompleteLogisticsDetailsFormWithCheckbox,
                      self).is_valid()
        if not valid: return valid
...
class CompleteCatalogueDetailsForm(ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'catalogue_about', 'catalogue_logo_squared',
        ...
        help_texts = {
            ...
            'catalogue_about':
            'Keep it concise – no more than 600 characters. Please write in english to reach both Swedish and international students.',
            #'catalogue_purpose': 'Keep it concise – no more than 600 characters.',
            'catalogue_contact_name':
            'This is the person that students will be referred to if they wish to get in touch with you outside of the career fair.',
            'catalogue_cities': 'E.g: Stockholm, Gothenburg, London'
        }

        labels = {
            'catalogue_about':
            'Text about your organisation (required to sign contract)',
            #'catalogue_purpose': 'Your organisation\'s purpose',
            'catalogue_logo_squared':
            'Upload your company\'s squared logotype. (required to sign contract)',
            'catalogue_logo_freesize':
            'Upload your company\'s logotype in any dimensions, in addition to the squared logotype.',
            'catalogue_cities':
            'Please write the main cities where your company operates. Separate the cities with commas.'
    ...
    def clean(self):
        ...
        if 'catalogue_contact_email_address' in self.cleaned_data and self.cleaned_data[
                'catalogue_contact_email_address'] is not None:
            self.cleaned_data[
                'catalogue_contact_email_address'] = self.cleaned_data[
                    'catalogue_contact_email_address'].lower()

        if 'catalogue_contact_phone_number' in self.cleaned_data:
            self.cleaned_data[
                'catalogue_contact_phone_number'] = fix_phone_number(
    ...
    def is_valid(self):
        ...
        if catalogue_contact_phone_number is not None and not re.match(
            ...
            self.add_error('catalogue_contact_phone_number',
...
class NewCompanyForm(ModelForm):
    def clean(self):
        ...
        if 'website' in self.cleaned_data:
            self.cleaned_data['website'] = fix_url(
    ...
    class Meta:
        model = Company
        fields = [
            'name', 'identity_number', 'website', 'type',
            'general_email_address'
        ]
...
class RegistrationForm(ModelForm):
    ...
    gdpr_accepted = BooleanField(required=True)
    gdpr_accepted.label = "THS Armada would like to process personal data about you and your organization to be able to contact you in conjunction with initial registration, complete registration and send you information regarding the fair of 2022. The data we intend to collect and the process is forename, surname, the title of your organization, phone number, and email address. You decide for yourself if you want to leave any additional information to us. The data will only be processed by the project group in Armada and will be saved in the Armada Internal Systems, AIS. You are, according to GDPR (General Data Protection Regulation), entitled to receive information regarding what personal data we process and how we process these. You also have the right to request a correction as to what personal data we are processing about you.\nI consent for THS Armada to process my personal data in accordance with the above.*"

    authorized_accepted = BooleanField(required=True)
    authorized_accepted.label = "I am authorized to register my company for Armada 2022 and sign this contract*"
...
class TransportForm(Form):
    contact_name = forms.CharField(
        ...
        help_text=
        'The person at your company who is responsible for the parcels to and from the fair.'
    )
    contact_email_address = forms.CharField(
        max_length=100,
        label='Contact person\'s e-mail address',
        help_text=
        'This is the e-mail address to which Ryska posten will respond.')
    ...
    description_of_parcels = forms.CharField(
        ...
        help_text=
        'Describe your parcels as detailed as possible; how many, their sizes, their weights etc. The description can be either in English or in Swedish.'
    )
    address_details = forms.CharField(
        widget=forms.Textarea,
        help_text=
        'Physical addresses that Ryska posten should collected from and deliver the parcels to.'
    )
...
class LunchTicketForm(ModelForm):
    ...
    class Meta:
        model = LunchTicket
        fields = [
            'email_address', 'comment', 'day', 'dietary_restrictions',
        ...
        help_texts = {
            ...
            'Please note that the lunch is entirely vegetarian.',
            'other_dietary_restrictions':
            'Please leave empty if no other restrictions.',
            'email_address':
            'The lunch ticket will be sent to this e-mail address in advance of the career fair.',
            'comment':
            'The comment is for your use only. It could, for instance, contain the name of the person who is going to use the ticket.'
        }
...
class BanquetParticipantForm(ModelForm):
    ...
    def is_valid(self):
        ...
        if phone_number is not None and not re.match(r'\+[0-9]+$',
                                                     phone_number):
    ...
    class Meta:
        ...
        fields = [
            'banquet', 'name', 'email_address', 'phone_number',
        ...
        help_texts = {
            ...
            'giveaway':
            'We plan to use this ticket in a giveaway for students.'
        }
