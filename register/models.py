...
class SignupContract(models.Model):
	...
	type = models.CharField(max_length = 200, choices = types, null = False, blank = False,
							default=types[0])
	contract_company_type = models.ForeignKey('companies.CompanyType', null = False, blank = False, on_delete = models.CASCADE)
	...
	def save(self, *args, **kwargs):
		...
		if self.current:
			...
			for contract in SignupContract.objects.filter(fair=self.fair, type=self.type, contract_company_type=self.contract_company_type, current=True):
				if contract != self:
		...
		if self.default:
			...
			for contract in SignupContract.objects.filter(fair=self.fair, type=self.type, default=True):
				if contract != self:
...
class SignupLog(models.Model):
	...
	def get_company_contact_company(self):
		if self.company_contact == None:
			return '-'
		else:
			return self.company_contact.company.name

	def __str__(self):
		return self.get_company_contact_company() + " for " + self.contract.name
