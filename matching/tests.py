...
class MatchingTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.resp_old = Response.objects.create(exhibitor=self.exhibitor_old, survey=self.survey_old)
        self.ans_old = TextAns.objects.create(question=self.q2016, response=self.resp_old)

        self.resp_current = Response.objects.create(exhibitor=self.exhibitor_current, survey=self.survey_current)
    ...
    def test_correct_filter(self):
        ...
        resp_current    = Response.objects.filter(survey=self.survey_current, exhibitor__in=[self.exhibitor_current])
        resp_old        = Response.objects.filter(survey=self.survey_old, exhibitor__in=[self.exhibitor_old])
