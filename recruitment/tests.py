...
class RecruitmentTestCase(TestCase):
    def setUp(self):
        ...
        self.purmonen_user = User.objects.create_user(
            username="purmonen", password="purmonen"
        )
        self.bratteby_user = User.objects.create_user(
            username="bratteby", password="bratteby"
        )
        self.core_user = User.objects.create_user(username="core", password="core")

        self.pg_role = Role.objects.create(name="PG", pk=1)
        self.core_group_role = Role.objects.create(
            name="Project Core Team", pk=33333333
        )
        self.system_developer = Role.objects.create(
            name="System Developer", parent_role=self.pg_role, pk=2
        )

        pg_permissions = [
            "view_recruitment_applications",
            "view_recruitment_interviews",
            "administer_recruitment_applications",
            "administer_roles",
            "administer_recruitment",
        ]

        for permission in pg_permissions:
            Group.objects.get(name="PG").permissions.add(
                Permission.objects.get(codename=permission)
            )

        self.career_fair_leader = Role.objects.create(name="Career Fair Leader", pk=3)

        self.recruitment_period.recruitable_roles.add(
            self.system_developer, self.career_fair_leader
        )

        self.recruitment_application = RecruitmentApplication.objects.create(
            pk=1337, user=self.purmonen_user, recruitment_period=self.recruitment_period
        )

        self.recruitment_application = RecruitmentApplication.objects.create(
            pk=2000, user=self.core_user, recruitment_period=self.recruitment_period
        )
    ...
    def test_recruitment_application(self):
        ...
        self.assertTrue(
            "recruitment.administer_recruitment"
            in self.purmonen_user.get_all_permissions()
        )
        self.assertTrue(
            "recruitment.administer_roles" in self.purmonen_user.get_all_permissions()
        )
    ...
    def test_site_for_non_armada_member(self):
        ...
        response = client.get(
            "/fairs/2016/recruitment/%d/application/1337" % self.recruitment_period.pk
        )
        self.assertEqual(response.status_code, 200)

        try:
            response = client.get(
                "/fairs/2016/recruitment/%d/application/1337/interview"
                % self.recruitment_period.pk
            )
        except (PermissionDenied, NoReverseMatch):
            pass
    ...
    def test_site_for_armada_member(self):
        client = Client()
        response = client.post(
            "/accounts/login/", {"username": "purmonen", "password": "purmonen"}
        )
        response = client.get("/fairs/2016/recruitment/")
        self.assertEqual(response.status_code, 200)
        ...
        response = client.post(
            "/accounts/login/", {"username": "core", "password": "core"}
        )
        response = client.get("/fairs/2016/recruitment/")
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_application(self):
        ...
        client = Client()
        self.assertEquals(
            len(self.recruitment_period.recruitmentapplication_set.all()), 2
        )
        ...
        response = client.post(
            "/fairs/2016/recruitment/%d/application/new" % self.recruitment_period.pk,
            {"username": "purmonen", "password": "purmonen"},
        )
        ...
        response = client.post(
            "/fairs/2016/recruitment/1/application/new",
            {
                "role1": "3",
                "programme": "1",
        ...
        self.assertEquals(
            len(self.recruitment_period.recruitmentapplication_set.all()), 3
        )

        recruitment_application = RecruitmentApplication.objects.get(
            user=self.bratteby_user
        )
        self.assertEquals(
            User.objects.get(username="bratteby").profile.phone_number, "0735307028"
        )
        self.assertEquals(
            RecruitmentApplication.objects.get(user=self.bratteby_user)
            .roleapplication_set.get(order=0)
            .role.pk,
            3,
        )

        response = client.post(
            "/fairs/2016/recruitment/1/application/%d" % recruitment_application.pk,
            {
                "role1": "2",
                "programme": "1",
                "registration_year": "2016",
        ...
        self.assertEquals(
            User.objects.get(username="bratteby").profile.phone_number, "0735307029"
        )
...
def check_users():
    users_with_roles = [
        (user, user.recruitmentapplication_set.filter(status="accepted").first())
    ...
    accepted_users_with_roles = [
        ...
        if user_with_role[1] and user_with_role[1].status == "accepted"
    ]
    len(accepted_users_with_roles)

    accepted_users_without_role_or_group = [
        user_with_role
        for user_with_role in accepted_users_with_roles
        if not user_with_role[1].delegated_role
        or not user_with_role[0]
        .groups.filter(name=user_with_role[1].delegated_role.name)
        .first()
    ]
    len(accepted_users_without_role_or_group)

    accepted_users_with_not_2_groups = [
        user for user in accepted_users_with_roles if len(user[0].groups.all()) != 2
    ]
