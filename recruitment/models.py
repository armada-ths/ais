...
def get_or_create_role_and_group(name, description=None, parent_role=None):
    group = Group.objects.get_or_create(name=name)[0]
    role = Role.objects.get_or_create(name=name, description=description, parent_role=parent_role)[0]
    role.group = group
...
def create_user_and_stuff(username, first_name, last_name, email, role_name, parent_role_name, recruitment_period_name, fair_name):
    ...
    if parent_role_name:
        (parent_role, parent_group) = get_or_create_role_and_group(parent_role_name, '')
    ...
    (role, group) = get_or_create_role_and_group(name=role_name, description='', parent_role=parent_role)
...
def create_armada_hosts():
    ...
    for area in hosts_areas:
        for role in area['roles']:
            get_or_create_role_and_group(role['name'], role['description'], host)
