...
class CompareForm(forms.Form):
    recruitment_periods = forms.ModelMultipleChoiceField(
        queryset=RecruitmentPeriod.objects.all(),
        widget=forms.CheckboxSelectMultiple(),
        required=True,
        label="Recruitment periods to compare",
    )
    include_late = forms.BooleanField(
        required=False, label="Include late applications, submitted after the end date"
    )
...
class RecruitmentApplicationSearchForm(forms.Form):
    ...
    submission_date = forms.CharField(
        widget=forms.TextInput(attrs={"class": "form-control"}), required=False
    )
    ...
    def applications_matching_search(self, application_list):
        ...
        if name:
            application_list = [
                ...
                if name.lower() in application.user.get_full_name().lower()
            ]
        ...
        if programme:
            application_list = [
                application
                for application in application_list
                if programme == application.user.profile.programme
            ]
        ...
        if submission_date:
            application_list = [
                application
                for application in application_list
                if submission_date.lower()
                in date_filter(
                    application.submission_date + timezone.timedelta(hours=2), "d M H:i"
                ).lower()
            ]
        ...
        if role and priority:
            application_list = [
                application
                for application in application_list
                if (role, int(priority))
                in [(roleapp.role, roleapp.order) for roleapp in application.roles]
            ]
        elif role:
            application_list = [
                application
                for application in application_list
                if role in [roleapp.role for roleapp in application.roles]
            ]
        elif priority:
            ...
            application_list = [
                application
                for application in application_list
                if int(priority) in [roleapp.order for roleapp in application.roles]
            ]
        ...
        if interviewer:
            application_list = [
                application
                for application in application_list
                if interviewer == application.interviewer
            ]
        ...
        if recommended_role:
            application_list = [
                application
                for application in application_list
                if recommended_role == application.recommended_role
            ]
        ...
        if rating:
            application_list = [
                application
                for application in application_list
                if int(rating) == application.rating
            ]
...
class RoleApplicationForm(forms.Form):
    role1 = forms.ModelChoiceField(
        label="Role 1",
        queryset=Role.objects.none(),
        widget=forms.Select(attrs={"required": True}),
    )
    role2 = forms.ModelChoiceField(
        label="Role 2", queryset=Role.objects.none(), required=False
    )
    role3 = forms.ModelChoiceField(
        label="Role 3", queryset=Role.objects.none(), required=False
    )
