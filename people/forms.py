...
class ProfileForm(ModelForm):
	...
	def is_valid(self):
		...
		if len(dietary_restrictions) == 0 and not other_dietary_restrictions and not no_dietary_restrictions:
			self.add_error('no_dietary_restrictions', 'If you have no dietary restrictions, tick this box to confirm.')
			valid = False

		elif (len(dietary_restrictions) != 0 or other_dietary_restrictions) and no_dietary_restrictions:
			self.add_error('no_dietary_restrictions', 'If you have dietary restrictions, you cannot tick this box. Leave the other dietary restrictions text field empty if not needed.')
			valid = False
