...
def start(request):
	...
	return render(request, "journal/start.html", {
		"fair": fair,
		"profile": profile,
		"form": form
	})
...
def ics(request, user_pk, token):
	...
	return render(request, "journal/ics.html", {
		"fair": fair,
		"interviews": (RecruitmentApplication.objects.filter(interviewer = user) | RecruitmentApplication.objects.filter(interviewer2 = user) | RecruitmentApplication.objects.filter(user = user)).all(),
		"profile": profile
