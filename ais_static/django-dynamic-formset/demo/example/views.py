...
def display_data(request, data, **kwargs):
    return render_to_response(
        "example/posted-data.html",
        dict(data=data, **kwargs),
...
def formset(request, formset_class, template):
    ...
    return render_to_response(
        template, {"formset": formset}, context_instance=RequestContext(request)
    )
...
def multiple_formsets(request, template):
    if request.method == "POST":
        contact_formset, event_formset = ContactFormset(
            request.POST, prefix="contact_form"
        ), EventFormset(request.POST, prefix="event_form")
        ...
        contact_formset, event_formset = ContactFormset(
            prefix="contact_form"
        ), EventFormset(prefix="event_form")
    return render_to_response(
        template,
        {"contact_formset": contact_formset, "event_formset": event_formset},
        context_instance=RequestContext(request),
    )
