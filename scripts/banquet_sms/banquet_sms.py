...
def make_message_for(attendee):
    ...
    name = name[0]
    message = (
        "Välkommen till banquet "
        + name
        + ", du sitter vid bord "
        + attendee["Table"]
        + " vid plats "
        + attendee["Placement Number"]
        + "."
    )
    return message
...
def main():
    ...
    with open(str(sys.argv[1]), "rb") as csvfile:
        ...
        for attendee in banquet_attendence:
            ...
            phone_number = "".join(
                _ for _ in attendee["Cell Phone Number"] if _ in "1234567890"
            )
            ...
            if send_to_this:
                ...
                error_file.write(
                    "Error with " + attendee["Name"] + "(id:" + attendee["Id"] + ")\n"
                )
                number_of_errors += 1
    ...
    print(
        "A total of %d text messages will be sent out. The price will be %.2f SEK"
        % (len(attendees), len(attendees) * 0.50)
    )
