...
faulty_orders = Order.objects.filter(
    exhibitor__in=current_exhibitor, product__in=not_current_products
)

for o in faulty_orders:
    company = o.exhibitor.company
    existing_exhibitor = Exhibitor.objects.filter(
        fair=Fair.objects.get(year=2016), company=company
    )

    if existing_exhibitor.exists():
        last_year_exhibitor = list(existing_exhibitor)[0]
    else:
        last_year_exhibitor = Exhibitor(
            fair=Fair.objects.get(year=2016),
            company=company,
            about_text="This exhibitor is incomplete due to a bug in 2017. The orders are complete.",
        )
