...
def lookup_user_with_api(kth_id):
	...
	retry_strategy = Retry(total=2, status_forcelist=[429, 500, 502, 503, 504], method_whitelist=["GET"])
...
def merge_user_info(user, info):
	...
	"""
	if info is not None and Profile.objects.filter(user = user, kth_synchronize = False).count() == 0:
		...
		if info['email'] is not None and (user.email is None or len(user.email) == 0 or user.email.endswith('@kth.se')):
