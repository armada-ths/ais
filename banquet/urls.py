...
urlpatterns = [
    ...
    url(r'^(?P<banquet_pk>[0-9]+)/invitations/(?P<invitation_pk>[0-9]+)/edit$', views.manage_invitation_form,
    ...
    url(r'^(?P<banquet_pk>[0-9]+)/participants/exportafterparty$', views.export_afterparty, name='banquet_export_afterparty'),
    url(r'^(?P<banquet_pk>[0-9]+)/participants/export$', views.export_participants, name='banquet_export_participants'),
    url(r'^(?P<banquet_pk>[0-9]+)/participants/(?P<participant_pk>[0-9]+)$', views.manage_participant, name='banquet_manage_participant'),
    url(r'^(?P<banquet_pk>[0-9]+)/participants/(?P<participant_pk>[0-9]+)/edit$', views.manage_participant_form,
        name='banquet_manage_participant_edit'),
    url(r'^(?P<banquet_pk>[0-9]+)/participants/(?P<participant_pk>[0-9]+)/remove$', views.manage_participant_remove,
