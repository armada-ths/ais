...
def ticket_search(request):
    ...
    name_query = reduce(operator.__or__, [
        Q(user__first_name__icontains=query) |
        Q(user__last_name__icontains=query) for query in names])
    ...
    data = {
        'result': [serializers.ticket(ticket) for ticket in tickets]
    }
...
def ticket_check_in_by_token(request):
    ...
    if participant is None:
        return JsonResponse({'message': 'Found no banquet ticket matching that code.'}, status=404)

    if participant.ticket_scanned is True:
        return JsonResponse({'message': 'Banquet ticket has already been used.'}, status=403)
