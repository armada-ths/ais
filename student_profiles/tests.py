...
class MatchingResultTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.company0 = Company.objects.create(
            name="TestCompany1", organisation_type="company"
        )
        self.company1 = Company.objects.create(
            name="TestCompany2", organisation_type="company"
        )
        self.exhibitor0 = Exhibitor.objects.create(
            company=self.company0, fair=self.fair
        )
        self.exhibitor1 = Exhibitor.objects.create(
            company=self.company1, fair=self.fair
        )
        ...
        for i in range(2):
            for j in range(2):
                mr = MatchingResult.objects.create(
                    fair=self.fair,
                    score=(i + j) * 10,
    ...
    def test_model(self):
        ...
        matchingresults_filtered = list(
            MatchingResult.objects.filter(
                fair=self.fair, exhibitor__in=[self.exhibitor0]
            )
        )
