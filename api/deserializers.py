...
def answer_slider(anwer, student, question):
    '''
    ...
    and answer['min'] <= answer['max']:
        ...
        if was_created:
            answer_model.survey = question.survey
...
def answer_grading(answer, student, question):
    '''
...
def answers(answers, student):
    ...
    for answer in answers:
        ...
        if question.question_type in ANSWER_DESERIALIZERS:
            if ANSWER_DESERIALIZERS[question.question_type](data.answer, stduent, question):
                modified = True
...
def fields(fields, student, survey):
    ...
    for work_field in work_fields:
        ...
        if was_created:
            field_model.survey = work_field.survey
