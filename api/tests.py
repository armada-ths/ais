...
class ExhibitorTestCase(TestCase):
    ...
    def test_serializer(self):
        serialized = serializers.exhibitor(
            self.request, self.exhibitor, self.exhibitor.company
        )
...
class StudentProfileTestCase(TestCase):
    ...
    def check_student_profile(
        self, profile, nickname=None, linkedin=None, facebook=None, phone_number=None
    ):
    ...
    def check_response(
        self, response, nickname=None, linkedin=None, facebook=None, phone_number=None
    ):
        self.assertEqual(response.status_code, HTTP_status_code_OK)
    ...
    def test_client(self):
        ...
        response = client.put(
            self.url_prefix + "?student_id=1", data=json.dumps({"nickname": "Postman"})
        )
        self.assertEqual(response.status_code, HTTP_status_code_OK)
    ...
    def test_put(self):
        ...
        request = self.factory.put(
            self.url_prefix + "?student_id=3", data=json.dumps({"nickname": "Mojo"})
        )
        response = views.student_profile(request)
        ...
        request = self.factory.put(
            self.url_prefix + "?student_id=49", data=json.dumps(["bleh bleh"])
        )
        response = views.student_profile(request)
        self.assertEqual(response.status_code, 406)

        request = self.factory.put(
            self.url_prefix + "?student_id=49",
...
class Organization(TestCase):
    def setUp(self):
        ...
        user2 = User.objects.create(
            username="user2", first_name="first", last_name="last"
        )
        user3 = User.objects.create(username="user3")
        programme = Programme.objects.create(name="Programme")
        profile1 = Profile.objects.create(
            user=user1, linkedin_url="url.url.se", programme=programme
        )
        profile2 = Profile.objects.create(
            user=user2,
            linkedin_url="url.url.se",
            picture_original="picture.original.url",
        )
        profile1.user = user1
        profile1.save()
        profile2.user = user2
    ...
    def test_view(self):
        ...
        self.assertEqual(
            organization[1]["people"][0]["picture"],
            "http://host/media/picture.original.url",
        )
        self.assertEqual(
            organization[0]["people"][0]["picture"],
            "http://host/static/images/no-image.png",
        )
...
class QuestionsTestCase(TestCase):
    def setUp(self):
        self.factory = RequestFactory()
        (self.fair, wasCreated) = Fair.objects.get_or_create(
            name="Armada fair", current=True
        )
        (self.survey, wasCreated) = Survey.objects.get_or_create(
            name="Dummy survey", fair=self.fair
        )

        # generate questions
        self.questions = [
            StudentQuestionSlider.objects.create(
                question="Question 1?", min_value=0.0, max_value=10000.0
            ),
            StudentQuestionSlider.objects.create(
            ...
            StudentQuestionGrading.objects.create(
                question="Wait what?", grading_size=5
            ),
        ]
        ...
        # generate areas
        (work_area, wasCreated) = WorkFieldArea.objects.get_or_create(
            work_area="Test area"
        )
        self.fields = [
            WorkField.objects.get_or_create(
                work_area=work_area, work_field="Test field"
            ),
            WorkField.objects.get_or_create(
                work_area=work_area, work_field="Another field"
            ),
    ...
    def test_put(self):
        ...
        def test_db():
            answers = StudentAnswerBase.objects.filter(
                student=StudentProfile.objects.get(pk=2)
            )
            answer_list = []
            field_list = []
            region_list = []
        ...
        data = json.dumps({"stuff": "nope"})
        request = self.factory.put("api/questions?student_id=2", data=data)
        response = views.questions(request)
        self.assertEqual(response.status_code, 406)
        test_db()

        data = json.dumps({"questions": "nope", "areas": "nope"})
        request = self.factory.put("api/questions?student_id=2", data=data)
        response = views.questions(request)
        self.assertEqual(response.status_code, 406)
        test_db()

        data = json.dumps({"questions": [], "areas": {"id": 0}})
        request = self.factory.put("api/questions?student_id=2", data=data)
        response = views.questions(request)
        self.assertEqual(response.status_code, 406)
        test_db()
        ...
        data = json.dumps({"regions": "fel"})
        request = self.factory.put("api/questions?student_id=2", data=data)
        response = views.questions(request)
        self.assertEqual(response.status_code, 406)

        # test partial payloads
        data = json.dumps({"areas": [], "regions": [], "continents": []})
        request = self.factory.put("api/questions?student_id=2", data=data)
        response = views.questions(request)
        self.assertEqual(response.status_code, 200)
        try:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            "1/5 question answers" in str(response.content)
        )  # only the last answer should be taken into account
        try:
            test_db()
        except AssertionError as error:
...
class BanquetPlacementTestCase(TestCase):
    def setUp(self):
        ...
        banquette_attendant1 = BanquetteAttendant.objects.create(
            first_name="Nr1", user=user, fair=current_fair
        )
        banquette_attendant2 = BanquetteAttendant.objects.create(
            first_name="Nr2", exhibitor=exhibitor, fair=current_fair
        )
        banquette_attendant_last = BanquetteAttendant.objects.create(
            first_name="Last", user=user, fair=last_fair
        )
...
class MatchingResultTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.exhibitor1 = Exhibitor.objects.create(
            company=self.company1, fair=current_fair
        )
        self.exhibitor2 = Exhibitor.objects.create(
            company=self.company2, fair=current_fair
        )
        self.exhibitor3 = Exhibitor.objects.create(
            company=self.company3, fair=current_fair
        )
        self.exhibitor4 = Exhibitor.objects.create(
            company=self.company4, fair=current_fair
        )
        self.exhibitor5 = Exhibitor.objects.create(
            company=self.company5, fair=current_fair
        )
        self.exhibitor6 = Exhibitor.objects.create(
            company=self.company6, fair=current_fair
        )
    ...
    def test_view(self):
        ...
        matching_result1 = MatchingResult.objects.create(
            student=self.student1,
            exhibitor=self.exhibitor1,
        ...
        matching_result2 = MatchingResult.objects.create(
            student=self.student1,
            exhibitor=self.exhibitor2,
            fair=Fair.objects.get(current=True),
            score=0,
        )
        matching_result3 = MatchingResult.objects.create(
            student=self.student1,
            exhibitor=self.exhibitor3,
            fair=Fair.objects.get(current=True),
            score=20,
        )
        matching_result4 = MatchingResult.objects.create(
            student=self.student1,
            exhibitor=self.exhibitor4,
            fair=Fair.objects.get(current=True),
            score=100,
        )
        matching_result5 = MatchingResult.objects.create(
            student=self.student1,
            exhibitor=self.exhibitor5,
            fair=Fair.objects.get(current=True),
            score=3,
        )
        request = self.factory.get("/api/matching_result?student_id=1")
        ...
        matching_result6 = MatchingResult.objects.create(
            student=self.student1,
            exhibitor=self.exhibitor6,
            fair=Fair.objects.get(current=True),
            score=70,
        )
        request = self.factory.get("/api/matching_result?student_id=1")
        ...
        matching_result7 = MatchingResult.objects.create(
            student=self.student2,
            exhibitor=self.exhibitor2,
            fair=Fair.objects.get(current=True),
            score=3,
        )
        matching_result8 = MatchingResult.objects.create(
            student=self.student2,
            exhibitor=self.exhibitor3,
            fair=Fair.objects.get(current=True),
            score=3,
        )
        matching_result9 = MatchingResult.objects.create(
            student=self.student2,
            exhibitor=self.exhibitor4,
            fair=Fair.objects.get(current=True),
            score=3,
        )
        matching_result10 = MatchingResult.objects.create(
            student=self.student2,
            exhibitor=self.exhibitor5,
            fair=Fair.objects.get(current=True),
            score=3,
        )
        matching_result11 = MatchingResult.objects.create(
            student=self.student2,
            exhibitor=self.exhibitor6,
            fair=Fair.objects.get(current=True),
            score=3,
